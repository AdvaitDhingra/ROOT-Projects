...
test('mock', function(t) {
  ...
  function opts(basedir) {
    return {
      ...
      isDirectory : function(dir) { return !!dirs[path.resolve(dir)]; },
      readFileSync : function(file) { return files[path.resolve(file)]; },
      realpathSync : function(file) { return file; }
    };
  }

  t.equal(resolve.sync('./baz', opts('/foo/bar')),
          path.resolve('/foo/bar/baz.js'));

  t.equal(resolve.sync('./baz.js', opts('/foo/bar')),
          path.resolve('/foo/bar/baz.js'));

  t.throws(function() { resolve.sync('baz', opts('/foo/bar')); });
...
test('mock package', function(t) {
  ...
  files[path.resolve('/foo/node_modules/bar/package.json')] =
      JSON.stringify({main : './baz.js'});
  ...
  t.equal(resolve.sync('bar', opts('/foo')),
...
test('readPackageSync', function(t) {
  ...
  function opts(basedir, useReadPackage) {
    return {
      ...
      readFileSync : useReadPackage
                         ? null
                         : function(file) { return files[path.resolve(file)]; },
      realpathSync : function(file) { return file; }
    };
  }
  t.test('with readFile', function(st) {
    ...
    st.equal(resolve.sync('bar', opts('/foo')),
             path.resolve('/foo/node_modules/bar/baz.js'));
  });
  ...
  t.test('with readPackage', function(st) {
    ...
    st.equal(resolve.sync('bar', options),
  ...
  t.test('with readFile and readPackage', function(st) {
    ...
    var options = opts('/foo');
    options.readPackageSync = readPackageSync;
    st.throws(function() { resolve.sync('bar', options); }, TypeError,
