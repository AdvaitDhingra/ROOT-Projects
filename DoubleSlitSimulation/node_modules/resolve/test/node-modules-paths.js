...
var verifyDirs = function verifyDirs(t, start, dirs, moduleDirectories, paths) {
  ...
  t.equal(keys(parsedDirs).length >= start.split(path.sep).length, true,
          'there are >= dirs than "start" has');
  var foundModuleDirNames = keys(foundModuleDirs);
  t.deepEqual(foundModuleDirNames, moduleDirs,
              'all desired module dirs were found');
  t.equal(keys(uniqueDirs).length, dirs.length,
...
test('node-modules-paths', function(t) {
  ...
  t.test('with paths=function option', function(t) {
    var paths = function paths(request, absoluteStart, getNodeModulesDirs,
                               opts) {
  ...
  t.test('with 1 moduleDirectory and paths options', function(t) {
    ...
    var dirs = nodeModulesPaths(
        start, {paths : paths, moduleDirectory : moduleDirectory});
  ...
  t.test('with 1+ moduleDirectory and paths options', function(t) {
    ...
    var dirs = nodeModulesPaths(
        start, {paths : paths, moduleDirectory : moduleDirectories});
  ...
  t.test('combine paths correctly on Windows', function(t) {
    ...
    t.equal(dirs.indexOf(path.resolve(start)) > -1, true,
  ...
  t.test('combine paths correctly on non-Windows',
         {skip : process.platform === 'win32'}, function(t) {
           var start = '/Users/username/git/myProject/src';
           var paths = [];
           var moduleDirectories =
               [ 'node_modules', '/Users/username/git/myProject/src' ];
           var dirs = nodeModulesPaths(
               start, {paths : paths, moduleDirectory : moduleDirectories});
