...
var packageDir = path.join(__dirname, 'resolver', 'symlinked', '_',
                           'node_modules', 'package');
var modADir =
    path.join(__dirname, 'symlinks', 'source', 'node_modules', 'mod-a');
var symlinkModADir =
    path.join(__dirname, 'symlinks', 'dest', 'node_modules', 'mod-a');
try {
...
try {
  ...
  // if fails then it is probably on Windows and lets try to create a junction
  fs.symlinkSync(path.join(__dirname, '..', '..', 'package') + '\\', packageDir,
                 'junction');
}
try {
  ...
  fs.symlinkSync(
      path.join(__dirname, '..', '..', 'source', 'node_modules', 'mod-a') +
          '\\',
      symlinkModADir, 'junction');
}

test('symlink', function(t) {
  t.plan(2);

  resolve('foo', {basedir : symlinkDir, preserveSymlinks : false},
          function(err, res, pkg) {
            t.error(err);
            t.equal(res, path.join(__dirname, 'resolver', 'symlinked', '_',
                                   'node_modules', 'foo.js'));
          });
});
...
test('sync symlink', function(t) {
  ...
  t.doesNotThrow(function() {
    t.equal(
        ...
        path.join(__dirname, 'resolver', 'symlinked', '_', 'node_modules',
                  'foo.js'));
  });
...
test('sync symlink when preserveSymlinks = true', function(t) {
  t.throws(function() { resolve.sync('foo', {basedir : symlinkDir}); },
              ...
              function(err, result) {
                ...
                t.equal(result,
                        path.resolve(__dirname,
                                     'resolver/symlinked/package/bar.js'));
              });
    });

test('packageFilter', function(t) {
  function relative(x) { return path.relative(__dirname, x); }
  ...
  function testPackageFilter(preserveSymlinks) {
    return function(st) {
      ...
      st.deepEqual(
          ...
          map(preserveSymlinks ? [destPkg, destPkg] : [sourcePkg, sourcePkg],
      path.normalize), 'sync: packageFilter pkgfile arg is correct'
      );
      */
      ...
      resolve('mod-a', {
        ...
        packageFilter : function(
            pkg, pkgfile) { asyncPackageFilterPath.push(pkgfile); }
      },
              function(err, actualPath) {
                ...
                st.deepEqual(map(asyncPackageFilterPath, relative),
                             map(preserveSymlinks
