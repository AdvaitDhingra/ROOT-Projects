...
module.exports = function resolve(x, options, callback) {
  ...
  if (opts.readFile && opts.readPackage) {
    var conflictErr =
        new TypeError('`readFile` and `readPackage` are mutually exclusive.');
    return process.nextTick(function() { cb(conflictErr); });
  }
    ...
    function load(exts, x, loadPackage) {
      ...
      function onpkg(err, pkg_, dir) {
        ...
        if (dir && pkg && opts.pathFilter) {
          ...
          if (r)
            return load([ '' ].concat(extensions.slice()), path.resolve(dir, r),
                        pkg);
        }
        isFile(file, onex);
      }
  ...
  function loadNodeModules(x, start, cb) {
    ...
    processDirs(cb, packageIterator ? packageIterator(x, start, thunk, opts)
                                    : thunk());
  }
};
